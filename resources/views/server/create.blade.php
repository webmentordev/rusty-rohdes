        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-2xl mb-3 font-semibold">Create new Servers</h2>
                    @if (session('success'))
                        <p class="p-4 bg-green-600/20 text-center mb-3 border border-green-600 text-green-700">{{ session('success') }}</p>
                    @endif
                    <form action="{{ route('server') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="w-full mb-3">
                            ...
                            <x-text-input id="ip" class="block mt-1 w-full" type="text" name="ip" :value="old('ip')" required />
                            <x-input-error :messages="$errors->get('ip')" class="mt-2" />
                        </div>

                        <div class="w-full mb-3">
                            <x-input-label for="port" :value="__('Server Port')" />
                            <x-text-input id="port" class="block mt-1 w-full" type="number" name="port" :value="old('port')" required />
                            <x-input-error :messages="$errors->get('port')" class="mt-2" />
                        </div>

                        <div class="w-full mb-3">
                            <x-input-label for="battlematric" :value="__('Battlematric ID')" />
                            <x-text-input id="battlematric" class="block mt-1 w-full" type="number" name="battlematric" :value="old('battlematric')" required />
